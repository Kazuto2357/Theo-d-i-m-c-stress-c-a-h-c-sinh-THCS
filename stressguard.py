...
# ====================== MIGRATION DỮ LIỆU CŨ (quan trọng!) ======================
if isinstance(st.session_state.data, list):
    old_entries = st.session_state.data
    st.session_state.data = {}
    if old_entries:
        st.session_state.data["6A1"] = old_entries  # Chuyển dữ liệu cũ sang lớp 6A1
        st.success("✅ Dữ liệu cũ đã được tự động chuyển sang lớp **6A1**. Bạn có thể xem ở tab Thống kê.")
...
class_data = st.session_state.data[current_class]
...
with tabs[0]:
    # ... (giữ nguyên code tab 1 cũ của bạn, mình đã copy đầy đủ)
...
with tabs[1]:
    # (đã có phần lưu vào lớp như trước)
...
# ==================== TAB Chatbot AI ====================
chat_tab_index = 4 if current_role == "Giáo viên chủ nhiệm" else 3
with tabs[chat_tab_index]:
    # (code chatbot giữ nguyên như cũ)
