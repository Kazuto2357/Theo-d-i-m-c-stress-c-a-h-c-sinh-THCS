...
with st.sidebar:
    ...
    if "user_key" in st.session_state and "student_name" in st.session_state:
        ...
        device_emoji = {"Điện thoại": "📱", "Máy tính bảng": "📟", "PC/Laptop": "💻"}.get(st.session_state.get("device_type", "PC/Laptop"), "💻")
        st.info(f"**Lớp:** {st.session_state.selected_class}\n"
                f"**Học sinh:** {st.session_state.student_name}\n"
                f"**Ngày sinh:** {birth_str}\n"
                f"**Thiết bị:** {device_emoji} {st.session_state.get('device_type', 'PC/Laptop')}")

    if st.button("🔄 Đổi thông tin cá nhân", use_container_width=True):
        for key in ["selected_class", "student_name", "birth_date", "user_key", "verified", "device_type", "messages"]:
...
# ====================== NHẬP THÔNG TIN CÁ NHÂN + THIẾT BỊ ======================
if "user_key" not in st.session_state:
    ...
    st.subheader("📱 Chọn thiết bị bạn đang sử dụng")
    device_type = st.radio(
        "Thiết bị",
        ["📱 Điện thoại", "📟 Máy tính bảng", "💻 PC/Laptop"],
        horizontal=True,
        label_visibility="collapsed"
    )
    device_type = device_type.split(" ", 1)[1]  # Lấy tên thiết bị sạch
...
if "verified" not in st.session_state or not st.session_state.verified:
    # (phần xác thực mật khẩu và quên mật khẩu giữ nguyên như code trước)
    if st.session_state.get("reset_mode", False):
        # ... (code quên mật khẩu giữ nguyên)
        st.subheader("🔑 Đặt lại mật khẩu")
        st.write("Xác minh danh tính để đặt mật khẩu mới")
        # (giữ nguyên code reset_mode từ file cũ)
        pass  # Bạn có thể copy phần reset_mode từ file trước nếu cần
    else:
...
device_type = st.session_state.get("device_type", "PC/Laptop")

# Điều chỉnh layout theo thiết bị
if device_type == "Điện thoại":
    st.set_page_config(layout="centered", page_title="AI Theo Dõi Mức Độ Căng Thẳng")
else:
    st.set_page_config(layout="wide", page_title="AI Theo Dõi Mức Độ Căng Thẳng")
...
# (Các tab 1, 2, 3, 4, 5 giữ nguyên như file trước của bạn – chỉ cần copy phần code tab từ file cũ vào đây)

# ==================== TAB 1: Nhật ký cá nhân ====================
with tab1:
    # (giữ nguyên code tab 1 của bạn)
...
# (Bạn copy tiếp phần tab 2, 3, 4, 5 từ file cũ của bạn vào đây – mình giữ nguyên logic như trước)
