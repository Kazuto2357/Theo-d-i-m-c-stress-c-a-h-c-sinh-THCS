...
if current_role == "Giáo viên chủ nhiệm":
    with tabs[3]:
        ...
        try:
            # Query join để lấy thêm thông tin từ profiles
            ...
            if response.data:
                ...
                if not df.empty:
                    df['date'] = pd.to_datetime(df['date'])
                    df['Ngày sinh'] = pd.to_datetime(df['birth_date']).dt.strftime('%d/%m/%Y')
                    df['Ngày ghi'] = df['date'].dt.strftime('%d/%m/%Y %H:%M')

                    # Bảng chi tiết đầy đủ
                    display_cols = [
                        'full_name', 'Ngày sinh', 'Ngày ghi', 
                        'mood', 'emotion', 'note', 'ai_advice'
                    ]
                    rename_cols = {
                        'full_name': 'Họ và tên',
                        'mood': 'Mức stress',
                    ...
                    # Biểu đồ histogram (giữ lại cho đẹp)
                    st.plotly_chart(
                        px.histogram(df, x='mood', title="Phân bố mức stress của lớp"), 
                        use_container_width=True
                    )

                    # Bảng chi tiết (có thể sort, search)
                    st.dataframe(
                        df_display,
                        use_container_width=True,
                        hide_index=True,
                    ...
                    # Nút xuất CSV (tiện cho giáo viên)
