...
# ====================== NHẬP THÔNG TIN CÁ NHÂN (CHỈ 1 LẦN) ======================
if "user_key" not in st.session_state:
    st.subheader("👤 Nhập thông tin cá nhân của bạn (chỉ làm 1 lần)")
    ...
    if st.button("✅ Xác nhận và bắt đầu sử dụng", type="primary", use_container_width=True):
        if not selected_class.strip() or not student_name.strip():
            ...
            st.success(f"✅ Chào {student_name}! Thông tin của bạn đã được lưu.")
            st.rerun()
    st.stop()
...
# (Các tab còn lại giữ nguyên như file bạn gửi, chỉ thay st.session_state.data → class_data khi append và save)
