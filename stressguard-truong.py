...
# Hiển thị form sửa hoặc tạo mới
if profile is None or st.session_state.get("edit_profile", False):
    ...
    if profile and profile.get("birth_date"):
        try:
            default_date = datetime.fromisoformat(profile["birth_date"].replace("Z", "+00:00")).date()
        except:
    ...
    current_role_index = role_options.index(profile.get("role")) if profile and profile.get("role") in role_options else 0
    role = st.radio("Vai trò", role_options, index=current_role_index)
