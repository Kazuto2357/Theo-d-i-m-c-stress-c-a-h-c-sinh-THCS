...
if isinstance(st.session_state.data, list):
    ...
    st.success("✅ Dữ liệu cũ đã được chuyển sang chế độ multi-user.")
...
with st.sidebar:
    ...
    if st.button("🔄 Đổi thông tin cá nhân", use_container_width=True):
        for key in ["selected_class", "student_name", "birth_year", "user_key", "messages"]:
...
# ====================== NHẬP THÔNG TIN CÁ NHÂN (TÀI KHOẢN RIÊNG) ======================
if "user_key" not in st.session_state:
    st.subheader("👤 Nhập thông tin cá nhân của bạn")
    
    col1, col2 = st.columns(2)
    with col1:
        selected_class = st.text_input("Lớp của bạn", placeholder="Ví dụ: 6A1, 7A2, 8A3...")
    with col2:
        birth_year = st.number_input("Năm sinh", min_value=2000, max_value=2015, value=2012, step=1)
    ...
    if st.button("✅ Xác nhận và bắt đầu sử dụng", type="primary", use_container_width=True):
        if not selected_class.strip() or not student_name.strip():
            st.error("❌ Vui lòng nhập đầy đủ Lớp và Họ tên!")
        else:
            # Tạo key riêng biệt cho mỗi người
            user_key = f"{selected_class.strip().upper()}_{student_name.strip().replace(' ', '_')}_{int(birth_year)}"
            st.session_state.selected_class = selected_class.strip().upper()
            st.session_state.student_name = student_name.strip()
            st.session_state.birth_year = int(birth_year)
            st.session_state.user_key = user_key
            st.success(f"✅ Chào {student_name}! Bạn đang sử dụng tài khoản riêng.")
            st.rerun()
    st.stop()
...
# Lấy dữ liệu cá nhân của user hiện tại
if user_key not in st.session_state.data:
    st.session_state.data[user_key] = []
class_data = st.session_state.data[user_key]

st.caption(f"📍 **{current_class}** — **{student_name}** (Tài khoản riêng)")
...
Đưa lời khuyên ngắn gọn, tích cực, dễ thực hiện bằng tiếng Việt."""
                    ...
                    st.session_state.data[user_key] = class_data
                    save_data(st.session_state.data)
...
with tab4:
    ...
    if password == "giao_vien_2026":
        # Hiển thị tất cả dữ liệu của mọi người (cho giáo viên)
        all_entries = []
        for key, entries in st.session_state.data.items():
            if isinstance(entries, list):
                all_entries.extend(entries)
        if all_entries:
            df = pd.DataFrame(all_entries)
            st.metric("Stress trung bình (toàn bộ)", f"{df['mood'].mean():.1f}/10" if len(df) > 0 else "0.0/10")
