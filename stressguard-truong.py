...
st.title("🧠 Phần Mềm AI Theo Dõi Mức Độ Căng Thẳng Học Sinh THCS")
...
# ====================== MẬT KHẨU GIÁO VIÊN ======================
TEACHER_PASSWORD = "giaovien@2026"
...
def load_profile():
    try:
        ...
        # User mới chưa có profile hoặc RLS tạm thời chặn → trả về None để hiển thị form
        return None
...
display_name = profile["full_name"] if profile and profile.get("full_name") else st.session_state.user.email
st.success(f"👋 Xin chào {display_name}!")

# ====================== MÀN HÌNH NHẬP MẬT KHẨU GIÁO VIÊN ======================
if st.session_state.get("show_teacher_pw", False):
    st.subheader("🔒 Xác nhận vai trò Giáo viên chủ nhiệm")
    st.info("Vai trò **Giáo viên chủ nhiệm** yêu cầu mật khẩu xác nhận.")
    teacher_pw_input = st.text_input("Nhập mật khẩu giáo viên", type="password", key="teacher_pw_input")
    col_pw1, col_pw2 = st.columns(2)
    with col_pw1:
        if st.button("✅ Xác nhận", type="primary", use_container_width=True):
            if teacher_pw_input == TEACHER_PASSWORD:
                data = st.session_state.pending_profile_data
                selected_class = data["class_name"]
                try:
                    check = supabase.table("profiles").select("id") \
                        .eq("role", "Giáo viên chủ nhiệm") \
                        .eq("class_name", selected_class) \
                        .neq("id", user_id).execute()
                    if check.data:
                        st.error(f"❌ Lớp {selected_class} đã có giáo viên chủ nhiệm! Vui lòng chọn lại lớp.")
                        del st.session_state.show_teacher_pw
                ...
                st.error("❌ Mật khẩu không đúng! Vui lòng chọn lại vai trò.")
                del st.session_state.show_teacher_pw
                del st.session_state.pending_profile_data
                st.rerun()
    with col_pw2:
...
# ====================== DANH SÁCH LỚP & HÀM HỖ TRỢ ======================
ALL_CLASSES = [f"{g}A{n}" for g in range(6, 10) for n in range(1, 8)]

def get_taken_teacher_classes():
    try:
        response = supabase.table("profiles").select("class_name") \
            .eq("role", "Giáo viên chủ nhiệm") \
            .neq("id", user_id).execute()
        return [r["class_name"] for r in response.data if r.get("class_name")]
    except:
        return []
...
if profile is None or st.session_state.get("edit_profile", False):
    ...
    # Chọn vai trò trước để quyết định UI chọn lớp
    ...
    # Chọn lớp theo vai trò
    if role == "Học sinh":
        grade_options = [6, 7, 8, 9]
        current_grade = int(profile["class_name"][0]) if profile and profile.get("class_name") and profile["class_name"][0].isdigit() else 6
        grade = st.selectbox("Chọn khối lớp", grade_options,
                             index=grade_options.index(current_grade) if current_grade in grade_options else 0)
        class_options = [f"{grade}A{n}" for n in range(1, 8)]
        current_class_val = profile.get("class_name", class_options[0]) if profile else class_options[0]
        class_default_idx = class_options.index(current_class_val) if current_class_val in class_options else 0
        class_name = st.selectbox("Chọn lớp", class_options, index=class_default_idx)
    else:
        taken_classes = get_taken_teacher_classes()
        available_classes = [c for c in ALL_CLASSES if c not in taken_classes]
        # Nếu đang sửa, giữ lại lớp hiện tại của GV trong danh sách
        if profile and profile.get("class_name") and profile["class_name"] not in available_classes:
            available_classes = [profile["class_name"]] + available_classes
        if not available_classes:
            st.error("❌ Tất cả 28 lớp đã có giáo viên chủ nhiệm!")
            class_name = ""
        else:
            current_class_val = profile.get("class_name", available_classes[0]) if profile else available_classes[0]
            class_default_idx = available_classes.index(current_class_val) if current_class_val in available_classes else 0
            class_name = st.selectbox("Chọn lớp chủ nhiệm", available_classes, index=class_default_idx)

    col1, col2 = st.columns(2)
    with col1:
        if st.button("✅ Lưu thông tin", type="primary", use_container_width=True):
            if not full_name.strip() or not class_name:
                ...
                if role == "Giáo viên chủ nhiệm":
                    st.session_state.show_teacher_pw = True
                    st.session_state.pending_profile_data = data
                    st.rerun()
                else:
                    try:
                        if profile is None:
